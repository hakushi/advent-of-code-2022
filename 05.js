...
parsedInstructions.forEach((instruction) => {
    for(i = 0;i<parseInt(instruction.amount, 10); i++) {
        const from = stackData[instruction.from].pop()
        if(from === undefined) {
            throw new Error(
                'undefined from data'
            )
        }
        if(!Object.hasOwn(stackData, instruction.to)) {
            stackData[instruction.to] = []
        }
        stackData[instruction.to].push(from)
    }
})
